...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    var medusa = new Medusa('Tofu')
    var first = new Person('Linda')
    var second = new Person('Dave')
    var third = new Person('Kumo')
    var fourth = new Person('Daft')

    medusa.stare(first)
    assert.equal(medusa.statues[0].name, 'Linda')
    medusa.stare(second)
    assert.equal(medusa.statues[1].name, 'Dave')
    medusa.stare(third)
    assert.equal(medusa.statues[2].name, 'Kumo')
    medusa.stare(fourth)
    assert.equal(medusa.statues[2].name, 'Daft')

    assert.equal(medusa.statues.includes('Linda'), false)
